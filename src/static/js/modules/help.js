...
export function renderEmptyState(context, container) {
    ...
    container.innerHTML = `
        <div class="text-center text-muted py-3 h-100 d-flex flex-column justify-content-center align-items-center">
            ...
            <h5 class="mt-2 fw-bold text-dark">${escapeHtml(info.title || '无项目')}</h5>
            ...
            ${info.steps ? `
                <div class="text-start bg-white p-3 rounded border shadow-sm small mx-auto" style="max-width: 350px;">
                    <h6 class="fw-bold mb-2 text-uppercase text-muted" style="font-size: 0.75rem;">操作步骤:</h6>
                    <ol class="ps-3 mb-0">
...
async function populateAboutModal() {
    ...
    if (data) {
        ...
        if (versionEl) versionEl.textContent = `版本: ${data.version || '未知'}`;
        ...
        const versionEl = document.getElementById('about-version');
        if (versionEl) versionEl.textContent = '版本: 不可用';
    }
}
...
function populateUpdateInstructions(instructions) {
    ...
    if (!instructions || !instructions.success) {
        ...
        if (titleEl) titleEl.textContent = '更新说明';
        if (sourceEl) sourceEl.textContent = '(无法获取最新更新说明)';
        if (stepsList) {
            stepsList.innerHTML = '<li>请访问 GitHub 发布页面查看更新说明。</li>';
        }
        return;
    }

    // Set title and source
    if (titleEl) titleEl.textContent = instructions.title || '更新说明';
    if (sourceEl) {
        const sourceText = instructions.source === 'remote' ? '(来源: GitHub 最新)' : '(来源: 本地缓存 ⚠️)';
...
async function handleCheckForUpdates() {
    ...
    if (!data.success) {
        ...
        if (errorLink) errorLink.href = data.releases_url || 'https://github.com/Jioyzen/zfdash-zh-cn/releases';
        return;
    }
    ...
    if (data.update_available) {
        ...
        if (currentBadge) currentBadge.textContent = `当前: v${data.current_version}`;
        if (latestBadge) latestBadge.textContent = `最新: v${data.latest_version}`;

        // Update releases link
        const releasesLink = document.getElementById('updates-releases-link');
        if (releasesLink) releasesLink.href = data.release_url || data.releases_url || 'https://github.com/Jioyzen/zfdash-zh-cn/releases';
