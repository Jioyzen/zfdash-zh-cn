...
export function setLoadingState(isLoading) {
    ...
    if (isLoading) {
        if (dom.statusIndicator) {
            dom.statusIndicator.innerHTML = `<span class="spinner-border spinner-border-sm text-light" role="status" aria-hidden="true"></span> 加载中...`;
        }
        if (dom.zfsTree) {
            dom.zfsTree.innerHTML = `<div class="text-center p-3"><span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> 加载中...</div>`;
        }
        ...
        if (dom.statusIndicator) {
            dom.statusIndicator.textContent = '就绪';
        }
    }
}
...
export function showModal(title, bodyHtml, onConfirm, options = {}) {
    ...
    if (options.footerHtml) {
        ...
        <button type="button" class="btn btn-primary" id="actionModalConfirmButton">确认</button>`;
...
export function showErrorAlert(title, message) {
    ...
    try {
        ...
        dom.errorModalBody.innerHTML = `
            ...
            <hr>
            <details>
                <summary class="text-muted small">复制错误详情</summary>
 ...
 */
export function showConfirmModal(title, htmlMessage, confirmBtnText = "确认", confirmBtnClass = "btn-primary", zIndex = null) {
 ...
 */
export function showInputModal(title, htmlMessage, defaultValue = '', placeholder = '', confirmBtnText = '确定', confirmBtnClass = 'btn-primary', zIndex = null) {
    return new Promise((resolve) => {
...
export function showDaemonDisconnectedOverlay(message, onReconnect = null, options = {}) {
    ...
    const helpHtml = showDaemonHelp ? `
                <p style="color: #555; font-size: 0.95rem; margin-bottom: 1rem;">
                    点击 <strong style="color: #6a64e8;">重新连接</strong> 重试，或启动守护进程：
                </p>
                <div class="mb-4 p-3" style="background: #2d2d3a; border-radius: 8px; font-family: var(--bs-font-monospace); font-size: 0.85rem;">
                    ...
                    <div style="color: #777; font-size: 0.8rem; margin-top: 6px;">或: uv run src/main.py --launch-daemon</div>
                </div>`
        :
        `<p style="color: #555; font-size: 0.95rem; margin-bottom: 1rem;">
                    点击 <strong style="color: #6a64e8;">重新连接</strong> 重试。
                </p>`;
    ...
    const switchAgentBtn = showSwitchAgentButton
        ? `<a href="/control-center" class="btn flex-fill py-2"
               ...
               <i class="bi bi-hdd-network-fill me-1"></i>切换代理
           </a>`
